...
/**
 * Removes an env from the list.
 * @this
 * @param {String} env environment to remove.
 */
proto.removeEnv = function(env) {
  var index = this.envs.indexOf(env);
...
proto.emit = function(event, data) {
  ...
  //on first run there will
  //be no env set it up.
  if (!currentEnv) {
    this.currentEnv = currentEnv = envId;
  }

  //when another env sends the start event queue
  //it to be next in line.
  if (event === this.START && currentEnv !== envId) {
...
/** exports */
module.exports = exports = ConcurrentReportingEvents;
