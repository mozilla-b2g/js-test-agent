...
BlanketConsoleReporter.prototype = {
  ...
  _onCoverageData: function _onCoverageData(data) {
    this.data = null;
    this.data = this._parseCoverageData(data);
    this._printConsoleFormat(this.data);
  },
  ...
  _printCoverageSummary: function _printCoverageSummary() {
    ...
    var passColor = '\u001b[0;32m',
        failColor = '\u001b[0;31m',
        originColor = '\u001b[0m',
        passes = this.totalPasses || 0,
        fails = this.totalFailures || 0,
        totals = passes + fails,
        output;
    ...
    console.log('Configure module coverage thresholds in ' +
      'build/config/test-agent-coverage.json.');
    console.log('For example: use { "system": 60 } to set up 60% ' +
      'module threshold for whole system app.' + '\n');
  ...
  getAppThreshold: function getAppThreshold(appName) {
    return parseInt(this.thresholds[appName]) || 0;
  },
