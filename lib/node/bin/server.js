...
if(argv['enable-http']){
  ...
  var httpServer = require('http').createServer(function (request, response) {
    ...
    function receiveFile(data){
      server.removeAllEventListeners('proxy ' + data.file);
    ...
    request.addListener('end', function () {
      ...
      Object.keys(proxies).forEach(function(key){
        if(url.indexOf(key) === 0){
          useProxy = true;
          proxySocket = proxies[key];
          relPath = url.slice(key.length);
        }
      });
      ...
      if(!useProxy){
        ...
        console.log('asking for ', relPath);
        server.on('proxy ' + relPath, receiveFile);
        proxySocket.send(server.stringify('proxy request', relPath));
      }
    });
  }).listen(argv.port);
  ...
  server.http.useProxy = function(data, socket){
    proxies[data.prefix] = socket;

    socket.on('close', function(){
      delete proxies[data.prefix];
    });
  };
